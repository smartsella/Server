...
const ForgotPassword = () => {
  ...
  const sendOtp = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || ''}/api/auth/forgot-password`, {
        ...
        body: JSON.stringify({ identifier, userType: "partner" }),
      });
  ...
  const verifyOtp = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || ''}/api/auth/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ identifier, otp, userType: "partner" }),
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-purple-100">
        ...
        {step === "EMAIL" ? (
          <form onSubmit={sendOtp} className="space-y-6">
            ...
            <button 
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded-xl font-semibold hover:scale-105 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
