...
const Signin = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    console.log('Partner login submitted:', formData)
    // Call backend login endpoint (use Vite env base URL when available so deployed frontend will call the backend)
    fetch(`${import.meta.env.VITE_API_BASE_URL || ''}/api/partners/login`, {
      method: 'POST',
      ...
      .then(async (res) => {
        ...
        const accommodations = body.data?.accommodations || []
        const services = body.data?.services || []

        console.log('Login response - user:', user)
        console.log('Login response - accommodations:', accommodations)
        console.log('Login response - services:', services)

        // Update context and localStorage
        if (user) {
          // Determine if this is an accommodation or service partner
          const isServicePartner = services.length > 0
          const isAccommodationPartner = accommodations.length > 0

          // Build basicDetails based on partner type
          let basicDetails = null

          if (isServicePartner) {
            // For service partners (Food, Laundry, etc.)
            ...
            basicDetails = {
              ...
              locationArea: service.location,
              serviceCategory: service.service_category,
              servicePricing: service.service_pricing,
            }
          } else if (isAccommodationPartner) {
            // For accommodation partners (PG, Hostel, Flat)
            basicDetails = {
              ownerName: accommodations[0].owner_name,
              email: accommodations[0].email_id,
              phoneNumber: accommodations[0].phone_number,
              propertyName: accommodations[0].property_name,
              locationArea: accommodations[0].location,
              roomsAvailable: accommodations[0].rooms_available,
              rules: accommodations[0].rules || {}
            }
          }

          // Merge user data with partner-specific data
          const partnerData = {
            ...user,
          ...
          console.log('Partner data being saved:', partnerData)

          login(partnerData)
          localStorage.setItem('user', JSON.stringify(partnerData))

          // Navigate to dashboard route from server or fallback to default
          const route = user?.dashboard_route || `/partner/accommodation/${(user.partner_type || 'pg-partner')}/dashboard`
          nav(route)
        } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-8 animate-fade-in-up">
          ...
          <p className="text-gray-600">Sign in to manage your services on Jeevigo</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-purple-100 animate-fade-in-up animation-delay-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="text-right">
              <NavLink to="/partner-forgot-password" className="text-sm text-blue-600 hover:text-blue-700 font-medium">
                Forgot password?
              </NavLink>
            </div>
